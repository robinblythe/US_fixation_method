...
total <- analysis |>
  filter(Strategy == "All patients receive cemented fixation") |>
...
print(paste0("The overall annual savings to Medicare of switching to cemented fixation are $",
             ceiling(sum(total$Cost)), " and ",
             ceiling(sum(total$QALY)), " QALYs."))
...
p_all +
  ...
  scale_y_continuous(labels = scales::unit_format(prefix = "$", unit = "M", scale = 1e-6),
                     limits = c(-20000000, 30000000),
                     breaks = seq(-20000000, 30000000, 10000000)) +
  labs(
    x = "QALYs",
    y = "Costs ($millions)",
